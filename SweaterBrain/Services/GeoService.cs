...
namespace SweaterBrain.Services
{
    internal class GeoService
    {
        ...
        public async Task<MajorCityLocationDto> GeoByPostalCodeAsync(string postalCode)
        {
            var requestUri = RequestUrl(postalCode);
        ...
        {
            return $"https://maps.googleapis.com/maps/api/geocode/json?sensor=true&components=country:US|postal_code:{postalCode}&key={_GOOGLE_KEY}&units=imperial";
        }
    }
}
