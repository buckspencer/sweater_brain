...
export class CityDropdown extends React.Component {
    ...
    render() {
        return (
            <Dropdown className="mt-3" isOpen={this.state.dropdownOpen} toggle={this.toggle}>
                <DropdownToggle caret>{this.state.value}</DropdownToggle>
                <DropdownMenu>
                    {this.state.locations.map((location) => (
                        <DropdownItem data-geo={ location.latLon } onClick={this.select} >{location.cityName}</DropdownItem>
                    ))}
                </DropdownMenu>
            </Dropdown>
        );
